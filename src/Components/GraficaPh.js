...
/*
  chart.subscribeCrosshairMove((param) => {
    if( param.point === undefined ||
      ...
      param.point.x < 0 ||
      param.point.y < 0){
        tooltipRef.display = 'none';
      }
      else{
        const data = param.seriesData.get(newSeries);
      const linePrice = param.seriesData.get(newSeries);

      const coordinate = newSeries.priceToCoordinate(linePrice.value);
